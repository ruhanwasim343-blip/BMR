import React from 'react'

export default function Services1() {
    return (
        <div className='bg-[#0F1C33] max-w-7xl px-6 py-12 mx-auto'>
            <div className='text-center'>
                <h1 className='text-2xl font-semibold text-white'>Our Services</h1>
                <p className='text-[9px] text-[#99A1AF] mt-4'>Comprehensive advisory services designed to guide your organization through every stage of <span className='block'>responsible AI adoption, from strategic planning to measurable outcomes.</span> </p>
            </div>


            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">

                {/* Card */}
                <div className="bg-[#15243D] border border-white/70 rounded-xl p-5 space-y-4">
                    <img className="w-8 h-8" src="/connectedcomp.svg" alt="" />
                    <h1 className='text-sm sm:text-base text-white font-semibold'>
                        AI Strategy & Governance
                    </h1>
                    <p className='text-xs sm:text-sm text-[#99A1AF] leading-relaxed'>
                        Develop comprehensive frameworks for responsible AI deployment,
                        risk management, and regulatory compliance.
                    </p>
                </div>

                {/* Repeat same card */}
                <div className="bg-[#15243D] border border-white/70 rounded-xl p-5 space-y-4">
                    <img className="w-8 h-8" src="/connectedcomp.svg" alt="" />
                    <h1 className='text-sm sm:text-base text-white font-semibold'>
                        AI Strategy & Governance
                    </h1>
                    <p className='text-xs sm:text-sm text-[#99A1AF] leading-relaxed'>
                        Develop comprehensive frameworks for responsible AI deployment,
                        risk management, and regulatory compliance.
                    </p>
                </div>

                <div className="bg-[#15243D] border border-white/70 rounded-xl p-5 space-y-4">
                    <img className="w-8 h-8" src="/connectedcomp.svg" alt="" />
                    <h1 className='text-sm sm:text-base text-white font-semibold'>
                        AI Strategy & Governance
                    </h1>
                    <p className='text-xs sm:text-sm text-[#99A1AF] leading-relaxed'>
                        Develop comprehensive frameworks for responsible AI deployment,
                        risk management, and regulatory compliance.
                    </p>
                </div>

                <div className="bg-[#15243D] border border-white/70 rounded-xl p-5 space-y-4">
                    <img className="w-8 h-8" src="/connectedcomp.svg" alt="" />
                    <h1 className='text-sm sm:text-base text-white font-semibold'>
                        AI Strategy & Governance
                    </h1>
                    <p className='text-xs sm:text-sm text-[#99A1AF] leading-relaxed'>
                        Develop comprehensive frameworks for responsible AI deployment,
                        risk management, and regulatory compliance.
                    </p>
                </div>

                <div className="bg-[#15243D] border border-white/70 rounded-xl p-5 space-y-4">
                    <img className="w-8 h-8" src="/connectedcomp.svg" alt="" />
                    <h1 className='text-sm sm:text-base text-white font-semibold'>
                        AI Strategy & Governance
                    </h1>
                    <p className='text-xs sm:text-sm text-[#99A1AF] leading-relaxed'>
                        Develop comprehensive frameworks for responsible AI deployment,
                        risk management, and regulatory compliance.
                    </p>
                </div>

                <div className="bg-[#15243D] border border-white/70 rounded-xl p-5 space-y-4">
                    <img className="w-8 h-8" src="/connectedcomp.svg" alt="" />
                    <h1 className='text-sm sm:text-base text-white font-semibold'>
                        AI Strategy & Governance
                    </h1>
                    <p className='text-xs sm:text-sm text-[#99A1AF] leading-relaxed'>
                        Develop comprehensive frameworks for responsible AI deployment,
                        risk management, and regulatory compliance.
                    </p>
                </div>

            </div>
        </div>
    )
}
